...
import { WakeWindow, useWakeSchedule } from '@/lib/useWakeSchedule'
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
        <span style={{ fontSize: '10px', color: T.text6, fontFamily: "'DM Mono', monospace", letterSpacing: '0.14em', textTransform: 'uppercase', width: '36px', flexShrink: 0 }}>Days</span>
        {DAY_LABELS.map((label, idx) => (
          <div key={idx} onClick={() => onToggleDay(idx)}
            style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        <span style={{ fontSize: '10px', color: T.text6, fontFamily: "'DM Mono', monospace", letterSpacing: '0.14em', textTransform: 'uppercase', width: '36px', flexShrink: 0 }}>Off at</span>
        <input type="time"
          value={`${win.offHour.toString().padStart(2,'0')}:${win.offMin.toString().padStart(2,'0')}`}
          onChange={e => {
            const [h, m] = e.target.value.split(':').map(Number)
            onChangeTime('offHour', h)
            onChangeTime('offMin', m)
          }}
          style={{ background: T.inputBg, border: `1px solid ${T.goldFaint7}`, color: T.text, padding: '6px 10px', outline: 'none', fontSize: '13px', fontFamily: "'DM Mono', monospace" }}
        />
        <span style={{ fontSize: '10px', color: T.text6, fontFamily: "'DM Mono', monospace" }}>→</span>
        <span style={{ fontSize: '10px', color: T.text6, fontFamily: "'DM Mono', monospace", letterSpacing: '0.14em', textTransform: 'uppercase' }}>On at</span>
        <input type="time"
          value={`${win.onHour.toString().padStart(2,'0')}:${win.onMin.toString().padStart(2,'0')}`}
          onChange={e => {
            const [h, m] = e.target.value.split(':').map(Number)
            onChangeTime('onHour', h)
            onChangeTime('onMin', m)
          }}
          style={{ background: T.inputBg, border: `1px solid ${T.goldFaint7}`, color: T.text, padding: '6px 10px', outline: 'none', fontSize: '13px', fontFamily: "'DM Mono', monospace" }}
        />
        <div style={{ marginLeft: 'auto' }}>
          <div onClick={onRemove} style={{ fontSize: '11px', color: T.red, cursor: 'pointer', padding: '4px 9px', border: `1px solid ${T.redBorder}` }}>Remove</div>
        </div>
      </div>

      {/* Summary */}
      <div style={{ fontSize: '11px', color: T.goldText2, fontFamily: "'DM Mono', monospace" }}>
        Wake word <span style={{ color: T.red }}>off</span> {fmt24to12(win.offHour, win.offMin)} → <span style={{ color: T.green }}>on</span> {fmt24to12(win.onHour, win.onMin)} · {describeDays(win.days)}
      </div>
    </div>
  )
}
    ...
    >
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontFamily: "'Playfair Display', serif", fontSize: '19px', fontStyle: 'italic', color: T.text, marginBottom: '4px' }}>
              Wake Word Schedule
            </div>
        ...
        {/* Status pill */}
        <div style={{ padding: '10px 24px', borderBottom: `1px solid ${T.borderFaint2}`, flexShrink: 0, background: T.inputBg }}>
          <div style={{
            ...
            <span style={{ fontSize: '11px', fontFamily: "'DM Mono', monospace", fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: scheduledOff ? T.red : T.green }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '18px 24px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {windows.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px 0', color: T.text7, fontSize: '13px', fontStyle: 'italic', fontFamily: "'Playfair Display', serif" }}>
